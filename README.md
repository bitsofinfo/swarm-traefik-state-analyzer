...
* [Modules/Scripts overview](#modules)
  ...
  * [servicechecker2prometheus.py](#servicechecker2prometheus)
...
1. `swarmstatedb.py`: to collect raw docker swarm service state into a JSON db
1. `servicechecksdb.py`: decorates all the layer service checks for services discovered by `swarmstatedb`
1. `servicechecker.py`: executes the service checks generated by `servicechecksdb`, records results
1. `servicecheckerreport.py`: reads `servicechecker` output and prepares a simple report  
1. `analyze-swarm-traefik-state.py`: orchestrates the above steps in one simple command
...
### Exposing metrics to Prometheus for analysis in Grafana
